...
class ResultUploadForm(forms.ModelForm):
    class Meta:
        model = Result
        fields = ['exam_number', 'student_name', 'year', 'term', 'category', 'class_name', 'remarks']
        widgets = {
            'exam_number': forms.TextInput(attrs={'placeholder': 'e.g. SCH2024001'}),
            ...
            'class_name': forms.TextInput(attrs={'placeholder': 'e.g. JSS 1A or SS 2B'}),
            'remarks': forms.Textarea(attrs={'rows': 2, 'placeholder': 'Optional remarks...'}),
        }

class StudentCheckForm(forms.Form):
    exam_number = forms.CharField(max_length=20, widget=forms.TextInput(attrs={'placeholder': 'e.g. SCH2024001'}))
